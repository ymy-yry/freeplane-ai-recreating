export const treeToFlow = (root) => {
    ...
    const visibleChildren = (node) => (node.folded ? [] : (node.children || []));
    const branchColor = (branchIndex) => BRANCH_COLORS[Math.abs(branchIndex) % BRANCH_COLORS.length];
    const alpha = (hex, value) => `${hex}${value}`;
    const subtreeSpan = (node) => {
        const children = visibleChildren(node);
        if (children.length === 0)
            return SIBLING_GAP;
        return Math.max(SIBLING_GAP, children.map(subtreeSpan).reduce((a, b) => a + b, 0));
    };
    const pushNode = (node, x, y, depth, branchIndex) => {
    ...
    const pushEdge = (sourceId, targetId, branchIndex) => {
        const color = branchColor(branchIndex);
        edges.push({
    ...
    const layoutBranch = (node, x, centerY, direction, depth, branchIndex, parentId) => {
        pushNode(node, x, centerY - NODE_HALF_HEIGHT, depth, branchIndex);
        if (parentId)
            pushEdge(parentId, node.id, branchIndex);
        const children = visibleChildren(node);
        if (children.length === 0)
            return;
        ...
        children.forEach((child) => {
            ...
            layoutBranch(child, x + direction * LEVEL_GAP, childCenterY, direction, depth + 1, branchIndex, node.id);
            cursorY += span;
        });
    };
    // 根节点始终居中
    ...
    const layoutSide = (sideChildren, direction, sideIndexOffset) => {
        ...
        sideChildren.forEach((child, index) => {
            const span = subtreeSpan(child);
            const childCenterY = cursorY + span / 2;
            layoutBranch(child, ROOT_X + direction * LEVEL_GAP, childCenterY, direction, 1, sideIndexOffset + index, root.id);
            cursorY += span;
        });
    };
