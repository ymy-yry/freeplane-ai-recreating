  ...
  <aside v-if="aiStore.panelVisible && aiStore.aiMode === 'build'" class="panel" :style="{ width: `${aiStore.panelWidth}px` }">
    ...
    <section class="card">
      ...
      <div class="row">
        ...
        <button :disabled="aiStore.buildLoading || !selectedNodeId" @click="runExpand">AI 展开</button>
      </div>
    </section>
    ...
    <section class="card">
      ...
      <div class="node">当前节点: {{ selectedNodeId || '请先选中节点' }}</div>
      <button :disabled="aiStore.buildLoading || !selectedNodeId" @click="runSummarize">生成摘要</button>
    </section>

    <section class="card">
      <div class="card-title">自动标签</div>
      <div class="node">当前节点: {{ selectedNodeId || '请先选中节点' }}</div>
      <button :disabled="aiStore.buildLoading || !selectedNodeId" @click="runTag">批量打标签</button>
    </section>
    ...
    <section class="result">
      ...
      <pre>{{ aiStore.buildResult || (aiStore.buildLoading ? '处理中...' : '暂无结果') }}</pre>
